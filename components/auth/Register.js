...
import { View, Text, StyleSheet, Platform, ScrollView, TouchableOpacity, StatusBar, KeyboardAvoidingView, Keyboard, TextInput, Image } from 'react-native';
...
export default class Register extends Component {
  ...
  registerPageSendController = () => {
    ...
    apiRequest({
      ...
      if (data.error && data.error == 'email_is_already_captured')
        return this.setState({ error: i18n.t('Sorry, this email is already registered') });
  ...
  render() {
    return (
      <KeyboardAvoidingView behavior={Platform.OS == "ios" ? "padding" : "height"} style={styles.main_wrapper}>
        <ScrollView>
          <TouchableOpacity activeOpacity={1} style={styles.content_wrapper} onPress={() => {Keyboard.dismiss()}} >
            <View style={styles.title_wrapper} >
              <Text style={styles.title} >{i18n.t('Test and ')}</Text>
              <Text style={[styles.title, {color: "rgb(240, 84, 79)"}]} >{i18n.t('earn!')}</Text>
            </View>
            <Image source={require('../../assets/login.png')} style={styles.login_image} ></Image>
            <TextInput
            ...
            <TouchableOpacity style={styles.start_button} onPress={() => {this.registerPageSendController()}} >
              <Text style={styles.start_text} >{i18n.t('Start Now')}</Text>
            </TouchableOpacity>
            <View style={styles.bottom_wrapper} >
              <Text style={styles.bottom_text} >{i18n.t('Are you a member?')}</Text>
              <TouchableOpacity onPress={() => {this.props.navigation.navigate('Login')}} >
                <Text style={styles.bottom_link} >{i18n.t('Login')}</Text>
              </TouchableOpacity>
            </View>
          </TouchableOpacity>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  title: {
    fontSize: 35, color: "rgb(30, 30, 30)", fontWeight: "800"
  },
  login_image: {
    width: "100%", resizeMode: "contain", marginBottom: 20
  },
  each_input: {
